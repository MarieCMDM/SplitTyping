...
import { fingerNames, getKey } from '../../domain/data'
import { calculateAccuracy, calculateWpm } from '../../domain/engine'
import Keyboard from '../../components/Keyboard'
import type { Attempt, Lesson, Settings } from '../../domain/types'

interface Props { lesson: Lesson; settings: Settings; onComplete:(attempt:Attempt)=>void; onExit:()=>void }

export default function Trainer({lesson,settings,onComplete,onExit}:Props) {
  ...
  const targetKey = getKey(target, settings.layout)
  ...
  return <main className="trainer" tabIndex={0} ref={box} onKeyDown={handleKey}>
    ...
    <section className="typing-stage">
      ...
      <div className={`hint ${wrong?'error-text':''}`}>{wrong ? <>Not <kbd>{wrong===' '?'Space':wrong}</kbd> — try again</> : target===' ' ? <>Use either <b>thumb</b> for Space</> : targetKey ? <>Use your <b>{fingerNames[targetKey.finger]}</b> for <kbd>{target}</kbd></> : <>Type <kbd>{target}</kbd></>}</div>
    </section>
    {settings.showKeyboard && <Keyboard kind={settings.keyboard} layout={settings.layout} target={target} wrong={wrong}/>}<p className="focus-note">Keep your eyes on the screen · Esc to leave</p>
  </main>
}
